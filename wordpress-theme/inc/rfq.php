...
function lulu_base_rfq_field($label, $name, $type = 'text', $required = false, $placeholder = '', $attributes = '') {
    ...
    <div class="form-field">
        ...
        <?php if ($type === 'textarea') : ?>
            <textarea id="<?php echo esc_attr($id); ?>" name="<?php echo esc_attr($name); ?>" rows="5" <?php echo $required ? 'required' : ''; ?> placeholder="<?php echo esc_attr($placeholder); ?>" <?php echo $attributes; ?>></textarea>
        <?php else : ?>
            <input id="<?php echo esc_attr($id); ?>" type="<?php echo esc_attr($type); ?>" name="<?php echo esc_attr($name); ?>" <?php echo $required ? 'required' : ''; ?> placeholder="<?php echo esc_attr($placeholder); ?>" <?php echo $attributes; ?>>
        <?php endif; ?>
    </div>
    <?php
}
...
function lulu_base_rfq_shortcode($atts = []) {
    ...
    <?php else : ?>
        <div class="rfq-form-bare">
            <?php lulu_base_rfq_notice(); ?>
    <?php endif; ?>
        <form class="rfq-form" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" method="post" enctype="multipart/form-data">
...
function lulu_base_handle_rfq() {
    ...
    if (!$full_name || !$company || !is_email($email) || !$country || !$consent) {
    ...
    $fields = [
        ...
        __('Grade / material', 'lulu-base')    => lulu_base_rfq_post_value('grade_material'),
