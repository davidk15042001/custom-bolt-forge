...
<section class="source-section"><div class="container">
    <div class="source-contact-tabs" data-source-tabs>
        <div class="source-tab-list" role="tablist">
            ...
            ?>
                <button id="source-tab-<?php echo esc_attr($tab[0]); ?>" type="button" role="tab" aria-selected="<?php echo esc_attr($index === 0 ? 'true' : 'false'); ?>" aria-controls="source-panel-<?php echo esc_attr($tab[0]); ?>" tabindex="<?php echo esc_attr($index === 0 ? '0' : '-1'); ?>" data-source-tab="<?php echo esc_attr($tab[0]); ?>"><?php echo esc_html($is_zh ? $tab[2] : $tab[1]); ?></button>
            <?php endforeach; ?>
        </div>
        <div class="source-contact-layout">
            <div class="source-contact-panels">
                <?php foreach ($tabs as $index => $tab) : ?>
                    <div id="source-panel-<?php echo esc_attr($tab[0]); ?>" class="source-contact-panel<?php echo $index === 0 ? ' is-active' : ''; ?>" role="tabpanel" aria-labelledby="source-tab-<?php echo esc_attr($tab[0]); ?>" data-source-panel="<?php echo esc_attr($tab[0]); ?>" <?php echo $index === 0 ? '' : 'hidden'; ?>>
            ...
            <aside class="source-contact-aside">
                <div class="source-info-card"><p class="eyebrow"><?php echo esc_html($is_zh ? '公司信息' : 'Company'); ?></p><p><strong><?php echo esc_html(get_bloginfo('name')); ?></strong></p><address><?php echo esc_html(lulu_base_option('address')); ?></address><p><a href="mailto:<?php echo esc_attr(lulu_base_option('email')); ?>"><?php echo esc_html(lulu_base_option('email')); ?></a></p><p><a href="tel:<?php echo esc_attr(preg_replace('/[^0-9+]/', '', (string) lulu_base_option('phone'))); ?>"><?php echo esc_html(lulu_base_option('phone')); ?></a></p></div>
