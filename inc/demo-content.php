...
function lulu_base_demo_admin_notice() {
    ...
    <div class="notice notice-info is-dismissible">
        <p>
            <?php if ($needs_sync) : ?>
                <?php esc_html_e('A newer content package is included in Xiangjinxin Industrial. Run the versioned migration to synchronize theme pages, products, translations and assets.', 'lulu-base'); ?>
            <?php else : ?>
                <?php esc_html_e('Xiangjinxin Industrial includes a complete site package. Import it from the theme settings page to create the pages, catalog, industries, RFQ content and images.', 'lulu-base'); ?>
            <?php endif; ?>
...
function lulu_base_demo_menu($location, $items, $sync = false) {
    $name = sprintf(__('Xiangjinxin %s', 'lulu-base'), ucfirst($location));
...
function lulu_base_demo_update_site_identity($company) {
    ...
    if (!$current_name || in_array($current_name, ['My WordPress', 'My WordPress Site', 'My WordPress Website', 'WordPress'], true)) {
        update_option('blogname', $name);
    }
    $current_description = get_option('blogdescription');
    if ((!$current_description || $current_description === 'Just another WordPress site') && $description) {
...
function lulu_base_demo_query_count($key) {
...
function lulu_base_render_demo_page() {
    ...
    <div class="wrap">
        ...
        <p><?php esc_html_e('This package includes the complete editable website content. Import it to create the pages, catalog, industries, RFQ routes, legal content and theme images.', 'lulu-base'); ?></p>
            ...
            <div class="notice notice-success inline">
                <p>
                    ...
                    printf(
                        ...
                        lulu_base_demo_query_count('demo_pages'),
                        lulu_base_demo_query_count('demo_updated'),
                        lulu_base_demo_query_count('demo_cats'),
                        lulu_base_demo_query_count('demo_products'),
                        lulu_base_demo_query_count('demo_menus'),
                        lulu_base_demo_query_count('demo_assets')
                    );
                    ?>
                </p>
            </div>
