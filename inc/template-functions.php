...
function lulu_base_default_options() {
    ...
    $defaults = [
        ...
        'footer_cta_url'          => '',
        'phone'                   => '+49 176 41474606',
        'email'                   => 'sales@xiangjinxin-fasteners.com',
        'address'                 => 'Row 2 No. 3, West Industrial Zone, Xisuining Village, Liuying Town, Yongnian District, Handan, Hebei, China',
        'whatsapp_number'         => '4917641474606',
        'whatsapp_message'        => __('Hello, I would like to discuss an industrial fastener enquiry.', 'lulu-base'),
        'linkedin_url'            => '',
        'facebook_url'            => '',
        'x_url'                   => '',
...
function lulu_base_privacy_url() {
    $url = trim((string) lulu_base_option('rfq_privacy_url'));
    if ($url === '') {
        $privacy_page_id = (int) get_option('wp_page_for_privacy_policy');
        return $privacy_page_id ? get_permalink($privacy_page_id) : home_url('/legal/privacy');
    }
    ...
    return wp_http_validate_url($url) ? $url : home_url('/legal/privacy');
}

function lulu_base_whatsapp_url() {
    $number = preg_replace('/[^0-9]/', '', (string) lulu_base_option('whatsapp_number'));
    if ($number === '') {
        return '';
    }

    $message = trim((string) lulu_base_option('whatsapp_message'));
    $url = 'https://wa.me/' . $number;
    return $message !== '' ? add_query_arg('text', $message, $url) : $url;
}
